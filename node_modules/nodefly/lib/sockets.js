function debug(format, args) {
	var args = ['SOCKET: ' + format].concat(args || []);
	if (/socket/.test(process.env.NODEFLY_DEBUG) ) {
...
module.exports.init = function(options) {
	...
	return {
		emit: function(){
			try {
				if (socket.socket.transport && socket.socket.transport.websocket && socket.socket.transport.websocket.readyState === 1) {
					debug('emitting data to collector');
					socket.emit.apply(socket,arguments)
				}
				else {
					debug('NOT emitting socket is not ready yet')
				}
			}
			catch(err) {
